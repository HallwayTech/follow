...
public class FollowApp {
	...
	if (fileFollowingPane != null) {
	    ...
	    fileFollowingPane = new FileFollowingPane(file, attributes
		    .getBufferSize(), attributes.getLatency(), attributes
		    .autoScroll(), attributes.getFont(), attributes
		    .getTabSize());
	    ...
	    tabbedPane.addTab(file.getName(), null, fileFollowingPane, file
		    .getAbsolutePath());
	    int tabCount = tabbedPane.getTabCount();
	    if (tabCount < 10) {
	...
	for (FollowAppAction a : actions.values()) {
		...
		a.setEnabled(false);
		    ...
		    && a.getContext() == ActionContext.SINGLE_FILE) {
		a.setEnabled(false);
	    } else {
		a.setEnabled(true);
	    }
